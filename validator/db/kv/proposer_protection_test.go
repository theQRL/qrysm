...
func TestProposalHistoryForSlot_InitializesNewPubKeys(t *testing.T) {
	pubkeys := [][field_params.DilithiumPubkeyLength]byte{{30}, {25}, {20}}
...
func TestNewProposalHistoryForSlot_ReturnsNilIfNoHistory(t *testing.T) {
	valPubkey := [field_params.DilithiumPubkeyLength]byte{1, 2, 3}
...
func TestSaveProposalHistoryForSlot_OK(t *testing.T) {
	pubkey := [field_params.DilithiumPubkeyLength]byte{3}
	db := setupDB(t, [][field_params.DilithiumPubkeyLength]byte{pubkey})
...
func TestSaveProposalHistoryForSlot_Overwrites(t *testing.T) {
	pubkey := [field_params.DilithiumPubkeyLength]byte{0}
	tests := []struct {
		signingRoot []byte
	}{
		{
...
func TestStore_ProposedPublicKeys(t *testing.T) {
	...
	require.NoError(t, err)
	assert.DeepEqual(t, make([][field_params.DilithiumPubkeyLength]byte, 0), keys)
	...
	keys, err = validatorDB.ProposedPublicKeys(ctx)
	require.NoError(t, err)
	assert.DeepEqual(t, [][field_params.DilithiumPubkeyLength]byte{pubKey}, keys)
}
