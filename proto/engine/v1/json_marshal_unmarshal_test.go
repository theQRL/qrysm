...
func legacy48AddressAs64(b []byte) []byte {
	out := make([]byte, fieldparams.FeeRecipientLength)
	copy(out[16:], bytesutil.PadTo(b, 48))
	return out
}
...
func TestJsonMarshalUnmarshal(t *testing.T) {
	...
	t.Run("execution payload Zond", func(t *testing.T) {
		...
		resp := &enginev1.GetPayloadV2ResponseJson{
			...
			ExecutionPayload: &enginev1.ExecutionPayloadZondJSON{
				...
				Withdrawals: []*enginev1.Withdrawal{{
					Index:          1,
					ValidatorIndex: 1,
					Address:        legacy48AddressAs64([]byte("address")),
					Amount:         1,
				}},
			},
		}
		...
		require.DeepEqual(t, legacy48AddressAs64([]byte("address")), withdrawal.Address)
