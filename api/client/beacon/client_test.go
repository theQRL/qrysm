...
func TestValidHostname(t *testing.T) {
	cases := []struct {
		...
		},
		{
			...
			joined:  "http://mydomain.org:3500/qrl/v1/node/version",
		},
		{
			name:    "https scheme, hostname with port",
			hostArg: "https://mydomain.org:3500",
			path:    getNodeVersionPath,
			joined:  "https://mydomain.org:3500/qrl/v1/node/version",
		},
		{
			name:    "http scheme, hostname without port",
			hostArg: "http://mydomain.org",
			path:    getNodeVersionPath,
			joined:  "http://mydomain.org/qrl/v1/node/version",
		},
		{
		...
		},
		{
			...
			path:    getNodeVersionPath,
			joined:  "http://username:pass@mydomain.org/qrl/v1/node/version",
		},
	}
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
