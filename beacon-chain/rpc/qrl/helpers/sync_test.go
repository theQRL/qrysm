...
func TestIsOptimistic(t *testing.T) {
	...
	t.Run("finalized", func(t *testing.T) {
		t.Run("finalized checkpoint is optimistic", func(t *testing.T) {
			st, err := util.NewBeaconStateZond()
			require.NoError(t, err)
			cs := &chainmock.ChainService{Optimistic: true, FinalizedCheckPoint: &qrysmpb.Checkpoint{}, OptimisticRoots: map[[32]byte]bool{{}: true}}
	...
	t.Run("justified", func(t *testing.T) {
		t.Run("justified checkpoint is optimistic", func(t *testing.T) {
			st, err := util.NewBeaconStateZond()
			require.NoError(t, err)
			cs := &chainmock.ChainService{Optimistic: true, CurrentJustifiedCheckPoint: &qrysmpb.Checkpoint{}, OptimisticRoots: map[[32]byte]bool{{}: true}}
			mf := &testutil.MockStater{BeaconState: st}
