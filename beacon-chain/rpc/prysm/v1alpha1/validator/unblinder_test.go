...
func Test_unblindBuilderBlock(t *testing.T) {
	...
	tests := []struct {
		...
		blindBlobs           []*zond.SignedBlindedBlobSidecar
		mock                 *builderTest.MockBuilderService
		err                  string
		returnedBlk          interfaces.SignedBeaconBlock
		returnedBlobSidecars []*zond.SignedBlobSidecar
	}{
		{
		...
		},
		{
			...
			blindBlobs: func() []*zond.SignedBlindedBlobSidecar {
				blobs := make([]*zond.SignedBlindedBlobSidecar, fieldparams.MaxBlobsPerBlock)
				for i := 0; i < fieldparams.MaxBlobsPerBlock; i++ {
					blobs[i] = &zond.SignedBlindedBlobSidecar{
			...
			blk: func() interfaces.SignedBeaconBlock {
				...
				b.Message.Body.DilithiumToExecutionChanges = []*zond.SignedDilithiumToExecutionChange{
					{
						Message: &zond.DilithiumToExecutionChange{
							ValidatorIndex:      123,
							FromDilithiumPubkey: []byte{'a'},
			...
			returnedBlobSidecars: func() []*zond.SignedBlobSidecar {
				blobs := make([]*zond.SignedBlobSidecar, fieldparams.MaxBlobsPerBlock)
				for i := 0; i < fieldparams.MaxBlobsPerBlock; i++ {
					blobs[i] = &zond.SignedBlobSidecar{
