...
type expectedDefiner func(t *testing.T, scs []*zondpb.BlobSidecar, req interface{}) []*expectedBlobChunk
type requestFromSidecars func([]*zondpb.BlobSidecar) interface{}
...
func generateTestBlockWithSidecars(t *testing.T, parent [32]byte, slot types.Slot, nblobs int) (*zondpb.SignedBeaconBlockDeneb, []*zondpb.BlobSidecar) {
	...
	sidecars := make([]*zondpb.BlobSidecar, len(commitments))
...
func generateTestSidecar(root [32]byte, block *zondpb.SignedBeaconBlockDeneb, index int, commitment []byte) *zondpb.BlobSidecar {
	blob := make([]byte, fieldparams.BlobSize)
	binary.LittleEndian.PutUint64(blob, uint64(index))
	sc := &zondpb.BlobSidecar{
		BlockRoot:       root[:],
		Index:           uint64(index),
...
func (c *blobsTestCase) setup(t *testing.T) (*Service, []*zondpb.BlobSidecar, func()) {
...
func (c *blobsTestCase) run(t *testing.T) {
	...
	m := map[types.Slot][]*zondpb.BlobSidecar{}
...
func defaultMockChain(t *testing.T) (*mock.ChainService, *startup.Clock) {
	...
	chain := &mock.ChainService{
		FinalizedCheckPoint: &zondpb.Checkpoint{Epoch: fe},
		Fork:                df,
	}
