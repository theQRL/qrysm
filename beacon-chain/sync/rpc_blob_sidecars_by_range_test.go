...
func blobRangeRequestFromSidecars(scs []*zondpb.BlobSidecar) interface{} {
	maxBlobs := fieldparams.MaxBlobsPerBlock
	count := uint64(len(scs) / maxBlobs)
	return &zondpb.BlobSidecarsByRangeRequest{
		StartSlot: scs[0].Slot,
		Count:     count,
	}
}

func (c *blobsTestCase) filterExpectedByRange(t *testing.T, scs []*zondpb.BlobSidecar, req interface{}) []*expectedBlobChunk {
	var expect []*expectedBlobChunk
	blockOffset := 0
	lastRoot := bytesutil.ToBytes32(scs[0].BlockRoot)
	rreq, ok := req.(*zondpb.BlobSidecarsByRangeRequest)
...
func TestBlobByRangeOK(t *testing.T) {
	...
	cases := []*blobsTestCase{
		...
		},
		{
			...
			nblocks: 10,
			requestFromSidecars: func(scs []*zondpb.BlobSidecar) interface{} {
				return &zondpb.BlobSidecarsByRangeRequest{
