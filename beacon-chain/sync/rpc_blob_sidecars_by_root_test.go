...
func blobRootRequestFromSidecars(scs []*zondpb.BlobSidecar) interface{} {
	req := make(p2pTypes.BlobSidecarsByRootReq, 0)
	for _, sc := range scs {
		req = append(req, &zondpb.BlobIdentifier{BlockRoot: sc.BlockRoot, Index: sc.Index})
	}
	return &req
}

func (c *blobsTestCase) filterExpectedByRoot(t *testing.T, scs []*zondpb.BlobSidecar, r interface{}) []*expectedBlobChunk {
	...
	scMap := make(map[[32]byte]map[uint64]*zondpb.BlobSidecar)
	for _, sc := range scs {
		...
		_, ok := scMap[root]
		if !ok {
			scMap[root] = make(map[uint64]*zondpb.BlobSidecar)
		}
...
func readChunkEncodedBlobsLowMax(t *testing.T, s *Service, expect []*expectedBlobChunk) func(network.Stream) {
	...
	require.NoError(t, err)
	vf := func(sidecar *zondpb.BlobSidecar) error {
		return nil
	}
...
func TestBlobsByRootValidation(t *testing.T) {
	...
	dmc.FinalizedCheckPoint = &zondpb.Checkpoint{Epoch: params.BeaconConfig().CapellaForkEpoch}
	cases := []*blobsTestCase{
		{
