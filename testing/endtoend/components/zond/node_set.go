...
// NodeSet represents a set of zond nodes, none of which is a mining node.
type NodeSet struct {
...
func (s *NodeSet) Start(ctx context.Context) error {
	// Create zond nodes. The number of nodes is the same as the number of beacon nodes.
	// We want each beacon node to connect to its own zond node.
	// We start up one zond node less than the beacon node count because the first
	// beacon node will connect to the already existing zond miner.
