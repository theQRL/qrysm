...
func TestEndToEnd_MainnetConfig_ValidatorAtCurrentRelease(t *testing.T) {
	e2eMainnet(t, true, types.StartAt(version.Zond, params.E2EMainnetTestConfig())).run()
}

func TestEndToEnd_MainnetConfig(t *testing.T) {
	e2eMainnet(t, false, types.StartAt(version.Zond, params.E2EMainnetTestConfig())).run()
}
