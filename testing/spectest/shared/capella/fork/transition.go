...
func RunForkTransitionTest(t *testing.T, config string) {
	...
	for _, folder := range testFolders {
		t.Run(folder.Name(), func(t *testing.T) {
			...
			preforkBlocks := make([]*zondpb.SignedBeaconBlockBellatrix, 0)
			postforkBlocks := make([]*zondpb.SignedBeaconBlockCapella, 0)
			...
			if config.ForkBlock == nil {
				...
				for i := 0; i <= *config.ForkBlock; i++ {
					...
					blockSSZ, err := snappy.Decode(nil /* dst */, blockFile)
					require.NoError(t, err, "Failed to decompress")
					block := &zondpb.SignedBeaconBlockBellatrix{}
					require.NoError(t, block.UnmarshalSSZ(blockSSZ), "Failed to unmarshal")
			...
			require.NoError(t, err, "Failed to decompress")
			beaconStateBase := &zondpb.BeaconStateBellatrix{}
