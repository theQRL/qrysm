...
func RunRegistryUpdatesTests(t *testing.T, config string) {
	...
	testFolders, testsFolderPath := utils.TestFolders(t, config, "zond", "epoch_processing/registry_updates/pyspec_tests")
	if len(testFolders) == 0 {
		t.Fatalf("No test folders found for %s/%s/%s", config, "zond", "epoch_processing/registry_updates/pyspec_tests")
	}
