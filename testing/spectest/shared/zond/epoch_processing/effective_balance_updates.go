...
func RunEffectiveBalanceUpdatesTests(t *testing.T, config string) {
	...
	testFolders, testsFolderPath := utils.TestFolders(t, config, "zond", "epoch_processing/effective_balance_updates/pyspec_tests")
	if len(testFolders) == 0 {
		t.Fatalf("No test folders found for %s/%s/%s", config, "zond", "epoch_processing/effective_balance_updates/pyspec_tests")
	}
