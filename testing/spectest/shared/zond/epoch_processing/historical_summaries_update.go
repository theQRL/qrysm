...
func RunHistoricalSummariesUpdateTests(t *testing.T, config string) {
	...
	testFolders, testsFolderPath := utils.TestFolders(t, config, "zond", "epoch_processing/historical_summaries_update/pyspec_tests")
	if len(testFolders) == 0 {
		t.Fatalf("No test folders found for %s/%s/%s", config, "zond", "epoch_processing/historical_summaries_update/pyspec_tests")
	}
